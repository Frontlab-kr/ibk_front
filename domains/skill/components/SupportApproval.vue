<template>
  <div class="ibk-payment-center">
    <div class="ibk-payment-center-head">
      <div class="pc">
        <div class="ibk-payment-center-head__title">승인문서 <strong>{{skillStore.myApprovalStatusList.paging.totalCount}}</strong></div>
      </div>
    ...
    <div class="mo">
      <div class="ibk-payment-center-head__title">
        <div>승인문서 <strong>{{skillStore.myApprovalStatusList.paging.totalCount}}</strong></div>
      </div>
    </div>
    <div class="ibk-payment-center-list">
      <SupportItem :item="item" v-for="(item,index) in skillStore.myApprovalStatusList.contents" :key="index" />
    </div>
...
const fetchList = () => {
  skillStore.fetchApprovalStatusList(params);
};
...
const hasMoreData = computed(() => {
  const total = skillStore.myApprovalStatusList.paging.totalCount || 0;
  const currentLoaded = skillStore.myApprovalStatusList.contents.length;
