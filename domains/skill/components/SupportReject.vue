<template>
  <div class="ibk-payment-center">
    <div class="ibk-payment-center-head">
      <div class="pc">
        <div class="ibk-payment-center-head__title">반려문서 <strong>{{skillStore.myRejectStatusList.paging.totalCount}}</strong></div>
      </div>
    ...
    <div class="mo">
      <div class="ibk-payment-center-head__title">
        <div>반려문서 <strong>{{skillStore.myRejectStatusList.paging.totalCount}}</strong></div>
      </div>
    </div>
    <div class="ibk-payment-center-list">
      <SupportItem :item="item" v-for="(item,index) in skillStore.myRejectStatusList.contents" :key="index" />
    </div>
...
const fetchList = () => {
  skillStore.fetchRejectStatusList(params);
};
...
const hasMoreData = computed(() => {
  const total = skillStore.myRejectStatusList.paging.totalCount || 0;
  const currentLoaded = skillStore.myRejectStatusList.contents.length;
