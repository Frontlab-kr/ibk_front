<template>
  <Dialog v-model:visible="visible" modal class="ibk-dialog ibk-dialog-apply">
    <template #header>
      ...
      <div v-if="isSaved" class="ibk-dialog-apply__title">스튜디오 이용 신청상세</div>
      <div v-else class="ibk-dialog-apply__title">스튜디오 이용 신청하기</div>
    </template>
...
const saveFacilityRequest = async () => {
  if(!isSaved.value || props.item.cancelable){
    ...
    try {
      ...
      if (result.success) {
        Utility.alert(`스튜디오 신청이 ${saveText.value}되었습니다`);
        if(isSaved.value) await refresh();
        closeDialog();
      } else {
        Utility.alert(`스튜디오 신청중 오류가 발생하였습니다.`);
      }
    } catch (e){
