<template>
    ...
    <div class="ibk-apply-center">
      <div class="ibk-apply-center-head">
        ...
        <div class="ibk-apply-center-head__filter">
          ...
          <Button label="검색" severity="secondary" outlined  @click="searchClassRoom"/>
        </div>
      </div>
      <div class="ibk-apply-center-list">
        <template v-if="store.classRoomRequestData?.contents.length > 0">
          <div v-for="(item, index) in store.classRoomRequestData?.contents" class="ibk-apply-center-list-item">
  ...
  <FacilityRequestDialog v-if="dialogParams.visible" v-model:visible="dialogParams.visible" :item="dialogParams.item" modal class="ibk-dialog ibk-dialog-apply" @refresh="searchClassRoom"/>
</template>
...
const loadMoreData = async () => {
  params.pageNo++;
  hasNext.value = await store.getClassRoomRequestList(params,false);
...
const cancelFacilityRequest = async (item) => {
  const isYesOrNo = await Utility.confirm('강의실 신청을 취소하시겠습니까?');
  if (isYesOrNo) {
    const res = await $request().mdfcFacilityRequestCancel(item);
    if (res && res.status === 200) {
      Utility.alert('강의실 신청이 취소되었습니다.');
      await searchClassRoom();
    }else {
...
const searchClassRoom = async () => {
  ...
  hasNext.value = await store.getClassRoomRequestList(params,true);
  store.classRoomRequestData.contents.forEach(x => {
    x.cancelable = (x.useYn == 'Y')  && (x.athzYn !== 'Y');
  })
}
